...
def word_to_encode(word):
    ...
    for c in word:
        ...
        if new_char:
            if c == ' ':
                space_index = len(characters)
            characters.append(c)

    for i in range(len(characters)):
        row = [0] * len(characters)
        row[i] = 1
        char_encodings.append(row)

    for c in word:
        for i in range(len(characters)):
            if c == characters[i]:
                x_train.append(char_encodings[i])
                y_train.append(char_encodings[i])

    y_train.append(char_encodings[space_index])
    y_train.pop(0)
    print(characters)

    return char_encodings, characters, x_train, y_train


char_encodings, index_to_char, x_train, y_train = word_to_encode(" Hello world")
